...
function getComment(id){
    ...
    xhr.onload = function(){
        if(xhr.status === 200){
            var comments = JSON.parse(xhr.responseText); //	get the response data as a string
            console.log('체크체크'+xhr.responseText);
            ...
            comments.map(function(comment){
                ...
                row.appendChild(td);
                td=document.createElement('td');
                td.textContent = comment.user.name; //이부분이 좀 했깔린다.
                row.appendChild(td);
                td = document.createElement('td');
                ...
                edit.addEventListener('click', function(){ //수정 클릭 시
                    ...
                    if(!newComment) alert('내용을 반드시 입력하세요!');
                    var xhr = new XMLHttpRequest();
                    xhr.onload = function(){
                        if(xhr.status === 200){
