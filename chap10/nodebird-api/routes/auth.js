...
router.post('/login', isNotLoggedIn, (req, res, next) => {
  passport.authenticate('local', (authError, user, info) => { //이부분은 passport/index.js와 연결됨
    if (authError) {                       //자세한건 책p374를 보자(그안의 localStrategy.js와 연결됨)
    ...
    return req.login(user, (loginError) => { //Passport는 req객체에 login,logout 메서드를 추가한다. 
      if (loginError) {              //req.login은 passport.serializeUser를 호출한다. req.login에
        console.error(loginError);   //제공하는 user객체가 serializeUser로 넘어가게 된다.
